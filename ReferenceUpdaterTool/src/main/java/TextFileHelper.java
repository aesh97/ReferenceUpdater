		...
		try {
			...
			for (int i = 0; i < references.size(); i++) {
				...
				if (prev_reference == null) {
					new_text = new_text.concat(text.substring(0, reference.get_left_bracket_location()))
							.concat(updated_references.get(i));
					prev_reference = reference;
				} else {
					new_text = new_text.concat(text.substring(prev_reference.get_right_bracket_location() + 1,
							reference.get_left_bracket_location())).concat(updated_references.get(i));
					prev_reference = reference;
				}
			}
			if (prev_reference != null) {
